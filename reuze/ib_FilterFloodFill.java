 ...
 * on http://www.javagaming.org forum and adapted to be more generic. Anti-aliasing
 ...
 * @since April 26, 2005
 */
public final class ib_FilterFloodFill {
  ...
  /** True, if anti-alised fill should be used. */
   ...
   */
  public ib_FilterFloodFill(Image imageToFill) {
   ...
   */
  public ib_FilterFloodFill(Image imageToFill, Image maskImage) {
    // sets image to fill
  ...
  /**
   * Returns true, if anti-aliased filling is used.
   * 
   * @return  True, for anti-aliased filling.
   */
  ...
  /**
   * Sets if anti-aliased filling is used.
   * 
   * @param antialiased  True, for anti-aliased filling.
   */
  ...
  /*
   * can be overridden to do color-class comparison
   */
  protected boolean equal(int sourceColor, int startColor) {
	  return sourceColor==startColor;
  }
  /**
  ...
  private void floodFill() { 
    ...
    while (index < linearNRTodo.size()) {
      ...
      if (y > 0) {
        ...
        int x = left;
        while (x <= right) {
          if (equal(maskImage[yOff + x], startColor)) {
            x = floodFill(x, y - 1);
          }
          else {
            ...
            if (antialiased) {
              ...
              if (!equal(antialiasedColor, startColor)) {
  ...
  private int floodFill2(int x, int y) { 
    ...
    while ((left >= 0) && (equal(maskImage[yOff + left], startColor)));
    // fill antialised if allowed
    if ((antialiased) && (left >= 0)) {
      int antialiasedColor = maskImage[yOff + left];
      antialiasedColor = (antialiasedColor >> 1) & 0x7F7F7F7F;
    ...
    while ((right < width) && (equal(maskImage[yOff + right], startColor)));
    // fill antialised if allowed
    if ((antialiased) && (right < width)) {
      int antialiasedColor = maskImage[yOff + right];
      antialiasedColor = (antialiasedColor >> 1) & 0x7F7F7F7F;
  ...
  private int fillColor2(int x, int y) {
	if (this.patternWidth==0) return this.fillColor;
    x /= this.patternWidth;
