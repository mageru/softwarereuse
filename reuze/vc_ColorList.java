 ...
 */
public class vc_ColorList implements Iterable<vc_e_Colors> {
    ...
    public static final vc_ColorList createFromARGBArray(int[] pixels, int num,
        ...
        List<vc_e_Colors> colors = new ArrayList<vc_e_Colors>();
        vc_e_Colors temp = vc_e_Colors.black.copy();
        for (int i = 0; i < num; i++) {
            int idx;
            if (uniqueOnly) {
                ...
                do {
                    ...
                    temp=new vc_e_Colors(pixels[idx]);
                ...
                idx = m_MathUtils.random(pixels.length-1);
                colors.add(new vc_e_Colors(pixels[idx]));
            }
        }
    ...
    public static final vc_ColorList createUsingStrategy(
            vc_i_ColorTheoryStrategy strategy, vc_e_Colors c) {
    ...
    protected List<vc_e_Colors> colors = new ArrayList<vc_e_Colors>();
     ...
     * @param colors
     */
    public vc_ColorList(Collection<vc_e_Colors> colors) {
     ...
     * @param colorArray
     */
    public vc_ColorList(vc_e_Colors... colorArray) {
        for (vc_e_Colors c : colorArray) {
            colors.add(c.copy());
        }
    }
     ...
     * @return itself
     */
    public vc_ColorList add(vc_e_Colors c) {
        colors.add(c.copy());
        return this;
    }
     ...
     * @return itself
     */
    public vc_ColorList addAll(Collection<vc_e_Colors> collection) {
    ...
    public vc_ColorList clusterSort(vc_a_CriteriaAccess clusterCriteria,
            ...
            boolean isReversed) {
        ArrayList<vc_e_Colors> sorted = new ArrayList<vc_e_Colors>(colors);
        Collections.sort(sorted, clusterCriteria);
        Collections.reverse(sorted);
        ArrayList<vc_e_Colors> clusters = new ArrayList<vc_e_Colors>();

        float d = 1;
        int i = 0;
        int num = sorted.size();
        for (int j = 0; j < num; j++) {
            vc_e_Colors c = sorted.get(j);
            if (c.getComponentValue(clusterCriteria) < d) {
                ArrayList<vc_e_Colors> slice = new ArrayList<vc_e_Colors>();
     ...
     */
    public boolean contains(vc_e_Colors color) {
        for (vc_e_Colors c : colors) {
            if (c.equals(color)) {
                return true;
            }
    ...
    public vc_e_Colors getAverage() {
        ...
        int num = colors.size();
        if (num > 0) {
            return new vc_e_Colors(r / num, g / num, b / num, a / num);
        } else {
            return null;
        }
    }
    ...
    public vc_ColorList getBlended(float amount) {
        vc_e_Colors[] clrs = new vc_e_Colors[colors.size()];
        for (int i = 0; i < clrs.length; i++) {
            vc_e_Colors c = colors.get(i > 0 ? i - 1 : clrs.length - 1);
     ...
     */
    public vc_e_Colors getDarkest() {
        vc_e_Colors darkest = null;
     ...
     */
    public vc_e_Colors getLightest() {
        vc_e_Colors lightest = null;
     ...
     */
    public Iterator<vc_e_Colors> iterator() {
     ...
     * @return itself
     */
    public vc_ColorList sortByComparator(Comparator<vc_e_Colors> comp,
            boolean isReversed) {
    ...
    public vc_ColorList sortByDistance(vc_i_Distance proxy, boolean isReversed) {
        ...
        vc_e_Colors root = getDarkest();

        // Remove the darkest color from the stack,
        // put it in the sorted list as starting element.
        ArrayList<vc_e_Colors> stack = new ArrayList<vc_e_Colors>(colors);
        stack.remove(root);
        ArrayList<vc_e_Colors> sorted = new ArrayList<vc_e_Colors>(colors.size());
        sorted.add(root);
        ...
        int sortedCount = 0;
        while (stack.size() > 1) {
            vc_e_Colors closest = stack.get(0);
            vc_e_Colors lastSorted = sorted.get(sortedCount);
            float distance = proxy.distanceBetween(closest, lastSorted);
            for (int i = stack.size() - 1; i >= 0; i--) {
                vc_e_Colors c = stack.get(i);
     ...
     * metrics).
     * 
     * @see #sortByProximityTo(vc_e_Colors, vc_i_Distance, boolean)
     * @param target
     *            color
     * @param isReversed
     ...
     * @return sorted list
     */
    public vc_ColorList sortByProximityTo(vc_e_Colors target, boolean isReversed) {
     ...
     * @return sorted list
     */
    public vc_ColorList sortByProximityTo(vc_e_Colors target,
