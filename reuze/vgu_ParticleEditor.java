...
public class vgu_ParticleEditor extends JFrame {
	Effects lwjglCanvas;
	JPanel rowsPanel;
	...
	ied_ParticleEffect effect = new ied_ParticleEffect();
	final HashMap<dg_ParticleEmitter, ParticleData> particleData = new HashMap();
	public vgu_ParticleEditor () {
	...
	public vgu_ParticleEditor (vg_i_Sprite awt) {
		super("Particle Editor");
		addWindowListener(new WindowAdapter() {
			public void windowClosed (WindowEvent event) {
				System.exit(0);
	...
	void reloadRows () {
		/*EventQueue.invokeLater(new Runnable() {
			...
			public void run () {*/
				...
				addRow(new vgu_SpawnPanel(emitter.getSpawnShape(), vgu_ParticleEditor.this));
				...
				addRow(new vgu_OptionsPanel(vgu_ParticleEditor.this));
				for (Component component : rowsPanel.getComponents())
					if (component instanceof vgu_EditorPanel) ((vgu_EditorPanel)component).update(vgu_ParticleEditor.this);
				rowsPanel.repaint();
				rowsPanel.validate();
			/*}
		});*/
		...
		if (data.icon == null && imagePath != null) {
			try {
				...
				if (file.exists())
					...
					url = vgu_ParticleEditor.class.getResource(imagePath);
