 ...
 * A page curl effect.
 */
public class ib_FilterCurl extends ib_a_FilterTransform {
...
*/
	...
	static class Sampler {
		...
		public Sampler( BufferedImage image ) {
			...
			inPixels = ib_a_Ops.getRGB( image, 0, 0, width, height, null );
		}
		...
		public int sample( float x, float y ) {
			...
			return i_MathImageUtils.bilinearInterpolate(xWeight, yWeight, nw, ne, sw, se);
		}
    ...
    public BufferedImage filter( BufferedImage src, BufferedImage dst ) {
        ...
        if ( dst == null ) {
			...
			dst = new BufferedImage(dstCM, dstCM.createCompatibleWritableRaster((int)transformedSpace.width, (int)transformedSpace.height), dstCM.isAlphaPremultiplied(), null);
		}
		...
		outX = (int) transformedSpace.position.x;
		...
		for (int y = 0; y < outHeight; y++) {
			for (int x = 0; x < outWidth; x++) {
				...
				int rgb = i_MathImageUtils.bilinearInterpolate(xWeight, yWeight, nw, ne, sw, se);
				int r = (rgb >> 16) & 0xff;
				int g = (rgb >> 8) & 0xff;
				int b = rgb & 0xff;
				float shade = out[2];
				...
				if ( out[3] != 0 )
					outPixels[x] = vc_ColorOps.combinePixels( rgb, inPixels[srcWidth*y + x], vc_ColorOps.NORMAL );
				else
					outPixels[x] = rgb;
			}
			setRGB( dst, 0, y, (int)transformedSpace.width, 1, outPixels );
		}
		return dst;
	}
