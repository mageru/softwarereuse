...
import reuze.dg_TelegramDispatcher;
import reuze.dg_EntityDomainRectangular;
import reuze.dg_a_EntityBase;
import reuze.dg_EntityBuilding;
import reuze.dg_EntityMover;
import reuze.dg_EntityObstacle;
import reuze.dg_EntityWall;
import reuze.ga_Geometry2D;
import reuze.pt_StopWatch;
...
public class dg_World {
	...
	private HashMap<Point, HashSet<dg_EntityBuilding>> building_parts;
	private HashMap<Point, HashSet<dg_EntityWall>> wall_parts;
	private HashMap<Point, HashSet<dg_EntityObstacle>> obstacle_parts;
	private HashMap<Point, HashSet<dg_EntityMover>> moving_parts;
	...
	public dg_World(int width, int height, double partitionSize, double partitionOverlap){
		...
		building_parts = new HashMap<Point, HashSet<dg_EntityBuilding>>();
		wall_parts = new HashMap<Point, HashSet<dg_EntityWall>>();
		obstacle_parts = new HashMap<Point, HashSet<dg_EntityObstacle>>();
		moving_parts = new HashMap<Point, HashSet<dg_EntityMover>>();
	}
	 ...
	 * @return the calculated pixel position
	 */
	public Point world2pixel(ga_Vector2D wxy, Point pxy){
	...
	public void addMover(dg_EntityMover me){
		...
		if(cspOn){
			Point part = getPartition(me.getPos().x, me.getPos().y);
			if(!moving_parts.containsKey(part))
	...
	private void checkPartitionMoves(){
		...
		for(dg_EntityMover mover : movers){
			...
			prevPart = new Point((int) m_MathFast.floor(prevPos.x / partSize), (int) m_MathFast.floor(prevPos.y / partSize));
			currPart = new Point((int) m_MathFast.floor(currPos.x / partSize), (int) m_MathFast.floor(currPos.y / partSize));
//			return new Point( (int) FastMath.floor(x / partSize), (int) FastMath.floor(y / partSize) );
	...
	private Point getPartition(double x, double y){
		return new Point( (int) m_MathFast.floor(x / partSize), (int) m_MathFast.floor(y / partSize) );
	}
