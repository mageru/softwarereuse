...
public class demoToxiColorThemeDisk  extends PApplet {
	...
	public void draw() {
	  ...
	  t.addRange(vc_ColorRange.BRIGHT, vc_e_Colors.newRandom(), random(0.02f, 0.05f));
	  ...
	  if (showDiscs) {
	    ...
	    list.sortByProximityTo(vc_e_Colors.getColor("WHITE"), new vc_DistanceRGB(), false);
	    swatches(list, 32, yoff);
	    yoff+=SWATCH_HEIGHT+10;
	  }
	}
	...
	void discs(vc_ColorList list) {
	  ...
	  for (int i = 0; i < NUM_DISCS; i++) {
	    vc_e_Colors c = list.get((int) random(numCols)).copy();
