...
import reuze.vc_ColorNames;
import reuze.vc_e_Colors;
import reuze.m_InterpolateValueZoomLens;
import reuze.ga_SplineB;
...
public void setup() {
  ...
  bg = vc_e_Colors.getColor("deepskyblue");
}
...
public void draw() {
  ...
  if (mousePressed) {
    ...
    if (mouseButton == LEFT) {
      ...
      vc_e_Colors c = vc_e_Colors.getColor(s);
      ...
      bg = vc_e_Colors.getColor(names.get(selectedColorID));
    }
  }
}
...
class ColorWorm {
  ...
  ColorWorm(ga_Vector2 origin, vc_e_Colors c) {
...
void drawColorPalette() {
  ...
  for (int i=0,num=names.size()-1; i<=num; i++) {
    ...
    float x2 = zoomLens.interpolate(0, width, (float)(i+1) / num);
    // convert the color into ARGB color format (32bit packed integer)
    fill(vc_e_Colors.getColor(names.get(i)).toARGB());
