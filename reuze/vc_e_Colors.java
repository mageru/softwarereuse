...
public class vc_e_Colors extends Color { 
	...
	public vc_e_Colors(int r, int g, int b, int a) {
		super(r&0xff,g&0xff,b&0xff,a&0xff);
	}
	public vc_e_Colors(int argb) {
		super(argb, true);
	}
	public vc_e_Colors(int rgb, int alpha) {
		this(rgb>>16, rgb>>8, rgb, alpha);
	}
	public vc_e_Colors(int r, int g, int b) {
		super(r,g,b);
	}
	public vc_e_Colors(int argb, boolean hasAlpha) {
		super(argb, hasAlpha);
	}
	public vc_e_Colors(float r, float g, float b, float a) {
		super(r,g,b,a);
	}
	public vc_e_Colors(float r, float g, float b) {
		super(r,g,b);
	}
	public static vc_e_Colors getColor(String name) {
		return new vc_e_Colors(vc_ColorNamesInt.getARGB(name));
	}
	public vc_e_Colors copy() {
		int i=this.getRGB();
		return new vc_e_Colors(i,true);
	}
     ...
     * @return new color
     */
    public static final vc_e_Colors newHSV(float h, float s, float v) {
        return new vc_e_Colors(getHSBColor(h, s, v).getRGB(),true);
    }

    public static final vc_e_Colors newHSVA(float h, float s, float v, float a) {
    	return new vc_e_Colors((getHSBColor(h, s, v).getRGB())&0xFFFFFF, (int)(a*255));
    }
    /**
     ...
     * @return random color
     */
    public static final vc_e_Colors newRandom() {
        return new vc_e_Colors(m_MathUtils.random(1f), m_MathUtils.random(1f),
     ...
     * @return new color.
     */
    public static final vc_e_Colors newGray(float gray) {
     ...
     * @return new color.
     */
    public static final vc_e_Colors newGrayAlpha(float gray, float alpha) {
        return new vc_e_Colors(gray,gray,gray,alpha);
    }
	public vc_e_Colors blend(vc_e_Colors c, float t) {
		int i=this.getRGB();
		int r=(i>>16)&0xff;
		int g=(i>>8)&0xff;
		int b=i&0xff;
		...
		b+=(bb-b)*t;
		a+=(aa-a)*t;
		return new vc_e_Colors(r,g,b,a);
	}
	public float distanceToRGB(vc_e_Colors c) {
        int i=this.getRGB();
		int r=(i>>16)&0xff;
		int g=(i>>8)&0xff;
		int b=i&0xff;
	...
	public float distanceToHSV(vc_e_Colors c) {
		int i=getRGB();
	...
	public vc_e_Colors invert() {
		getComponents(temp); //rgb
        temp[0] = 1 - temp[0];
        temp[1] = 1 - temp[1];
        temp[2] = 1 - temp[2];
        return new vc_e_Colors(temp[0], temp[1], temp[2], temp[3]);
    }
	/**
     ...
     * @return itself
     */
    public vc_e_Colors saturate(float step) {
    	int i=getRGB();
    	float[] hsv=RGBtoHSB((i>>16)&0xff, (i>>8)&0xff, i&0xff, temp);
        hsv[1] += step;
        return new vc_e_Colors(HSBtoRGB(hsv[0],hsv[1],hsv[2]),getAlpha());
    }
    /**
     ...
     * @return itself
     */
    public vc_e_Colors lighten(float step) {
    	int i=getRGB();
    	float[] hsv=RGBtoHSB((i>>16)&0xff, (i>>8)&0xff, i&0xff, temp);
        hsv[2] += step;
        return new vc_e_Colors(HSBtoRGB(hsv[0],hsv[1],hsv[2]),getAlpha());
    ...
    public vc_e_Colors getRotatedRYB(int angle) {
        return rotateRYB(angle);
    }
    public vc_e_Colors rotateRYB(float theta) {
     ...
     * @return itself
     */
    public vc_e_Colors rotateRYB(int theta) {
    	int i=getRGB();
    	float[] hsv=RGBtoHSB((i>>16)&0xff, (i>>8)&0xff, i&0xff, temp);
        float h = hsv[0] * 360;
        theta %= 360;
    ...
    public vc_e_Colors complement() {
        return rotateRYB(180);
    }
    public vc_e_Colors setBrightness(float brightness) {
    	int i=getRGB();
    	float[] hsv=RGBtoHSB((i>>16)&0xff, (i>>8)&0xff, i&0xff, temp);
        hsv[2] = m_MathUtils.clip(brightness, 0, 1);
        Color c=getHSBColor(hsv[0],hsv[1],hsv[2]);
        return new vc_e_Colors(c.getRGB()&0xffffff,getAlpha());
    }
    public vc_e_Colors setSaturation(float saturation) {
    	int i=getRGB();
    	float[] hsv=RGBtoHSB((i>>16)&0xff, (i>>8)&0xff, i&0xff, temp);
     ...
     * @return itself
     */
    public vc_e_Colors analog(float theta, float delta) {
        return analog((int) m_MathUtils.degrees(theta), delta);
    }

    public vc_e_Colors analog(int angle, float delta) {
        vc_e_Colors c=rotateRYB((int) (angle * m_MathUtils.normalizedRandom()));
        int i=c.getRGB();
    	float[] hsv=RGBtoHSB((i>>16)&0xff, (i>>8)&0xff, i&0xff, temp);
	...
	public static final vc_e_Colors black=new vc_e_Colors(0);
}
