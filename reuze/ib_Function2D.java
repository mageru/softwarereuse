...
public class ib_Function2D implements m_i_Function2D {
	...
	public ib_Function2D(BufferedImage image) {
		this(image, false);
	}
	
	public ib_Function2D(BufferedImage image, boolean alpha) {
		this(image, ZERO, alpha);
	}
	
	public ib_Function2D(BufferedImage image, int edgeAction, boolean alpha) {
		init( getRGB( image, 0, 0, image.getWidth(), image.getHeight(), null), image.getWidth(), image.getHeight(), edgeAction, alpha);
	}
	
	public ib_Function2D(int[] pixels, int width, int height, int edgeAction, boolean alpha) {
		init(pixels, width, height, edgeAction, alpha);
	}
	
 	public ib_Function2D(Image image) {
		this( image, ZERO, false );
 	}
	
	public ib_Function2D(Image image, int edgeAction, boolean alpha) {
	...
	public float evaluate(float x, float y) {
		...
		int iy = (int)y;
		if (edgeAction == WRAP) {
			ix = m_MathUtils.mod(ix, width);
			iy = m_MathUtils.mod(iy, height);
		...
		return alpha ? ((pixels[iy*width+ix] >> 24) & 0xff) / 255.0f : vc_ColorOps.brightness(pixels[iy*width+ix]) / 255.0f;
	}
