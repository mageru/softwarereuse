...
public class m_InterpolateValueZoomLens implements m_i_InterpolateValue {
    ...
    public m_InterpolateValueZoomLens(float lensPos, float lensStrength) {
        ...
        leftImpl.setParameter("flip", lensStrength > 0?1f:0f);
        rightImpl.setParameter("flip", lensStrength < 0?1f:0f);
    }
	...
	public float setParameter(String name, float value) {
		float old;
		if (name.charAt(0)=='p') {old=lensPos; setLensPos(value, smooth);}
		else if (name.charAt(1)=='t') {old=lensStrength; setLensStrength(value, smooth);}
		else {old=smooth; smooth=value;}
		return old;
	}
