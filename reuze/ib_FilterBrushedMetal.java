 ...
 */
public class ib_FilterBrushedMetal implements BufferedImageOp {
    ...
    public ib_FilterBrushedMetal() {
    }
    
    public ib_FilterBrushedMetal( int color, int radius, float amount, boolean monochrome, float shine) {
        this.color = color;
        this.radius = radius;
        this.amount = amount;
	 ...
	 */
	public int[] getRGB( BufferedImage image, int x, int y, int width, int height, int[] pixels ) { //RPC
    ...
    public BufferedImage filter( BufferedImage src, BufferedImage dst ) {
        ...
        int[] pixels = new int[width];     //RPC added source pixel option if color==0
		...
		for ( int y = 0; y < height; y++ ) {
			if (color==0) getRGB( src, 0, y, width, 1, pixels );  //RPC
    ...
    public Rectangle2D.Float getBounds2D( BufferedImage src ) {
