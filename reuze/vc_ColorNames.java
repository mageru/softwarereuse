...
public class vc_ColorNames extends vc_ColorNamesInt {
    ...
    public static String getName(vc_e_Colors color) {
		int i,c=color.getRGB()&0xffffff;
	...
	public static vc_e_Colors getColor(String name, float alpha) {
		if ((name==null)||(name.length()==0)) return vc_e_Colors.black;
		...
		if (z=='#') {
			if (name.length()==4) {
				...
				return new vc_e_Colors(rx*0x11,gx*0x11,bx*0x11,(int)(alpha*255f));
			}
			...
			if (!pct) return new vc_e_Colors(rr/255f, gg/255f, bb/255f, alpha);
			return new vc_e_Colors(rr/100.0f,gg/100.0f,bb/100.0f,alpha);
		} else
			i=name2color.get(name.toLowerCase()).intValue();
        return new vc_e_Colors(
						 ((i>>16)&0xff)/255f,
						 ((i>>8)&0xff)/255f,
						 (i&0xff)/255f, alpha
						 );
